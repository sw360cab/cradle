...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
       ...
       _merge: function (id, doc, callback) {
        ...
        spatiallist: function (path, options) {
            var args = new(Args)(arguments);
            path = path.split('/');
            ...
            this.query('GET', ['_design', path[0], '_spatiallist', path[1], path[2]].map(querystring.escape).join('/'), options, args.callback);
        },
